...
function doExec(){
    ...
    if($lang == "j"){
        ...
        // C code
        $file_to_write =  $questionName . '.py';
        $content_to_write = getContentForPython($user, $questionName);
	...
	$c_var = 0;
        ...
        if($c_var == 0 ){
            ...
            //execute Python
            //CHECK THIS !!
            $cmd = 'python3 ' .  './'.$dir . '/' . $user . '/' . $file_to_write . ' 2> ./'.$dir . '/' . $user . '/' .$questionName.'.txt';
            //echo $cmd;
            $result =(exec($cmd, 
                           $r_output,
                           $r_var) );
        ...
        }else{ // useless because there is no compilation (Sami)
            $myfile = fopen('./'.$dir . '/' . $user . '/' .$questionName.'.txt', "r");// or die("Unable to open file!");
            // Output one line until end-of-file
            while(!feof($myfile)) {
                $cmd .= fgets($myfile) . "<br>";
...
function getContentForPython($user, $questionName){

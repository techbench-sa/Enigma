...
getScore: (id) => {
    ...
    getChallenge: id => {
        return new Promise((resolve, reject) => {
            ...
            db.query(`SELECT * FROM challenges WHERE number=${id}`, (err, res) => {
    ...
    addChallenge: data => {
        return new Promise((resolve, reject) => {
            ...
            db.query(`INSERT INTO challenges (name, description, method_name, method_type, tests, parameters, points) VALUES ('${name}', '${description}', '${method_name}', '${method_type}', '${tests}', '${parameters}', ${points})`,
            (err, res) => {
    ...
    addSubmission: ({ id, number, code, score }) => {
        return new Promise((resolve, reject) => {
            db.query(`UPDATE submissions SET score=${score} WHERE playerID=${id} and challengeNumber=${number}`, (err, res) => {
                if (err) {
                    reject()
                } else if (res.affectedRows == 0) {
                    db.query(`INSERT INTO submissions (playerID, challengeNumber, code, score) VALUES (${id}, ${number}, '${code}', ${score})`, (err, res) => {
